    ...
    <section className="bg-trizen-light py-12 md:py-16 lg:py-24">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
            ...
            <Tabs defaultValue="teaching" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="space-y-4 md:space-y-6 mb-6 md:mb-8">
                {tabContent.map((tab) => (
                  ...
                  >
                    <h1 className="font-heading font-bold tracking-tight text-trizen-dark text-3xl md:text-4xl lg:text-5xl mb-4 md:mb-6 transition-all duration-300">
                      {tab.heading}
                    </h1>
                  </TabsContent>
                ))}
              ...
              <Button 
                className="rounded-full px-5 py-5 md:px-6 md:py-6 bg-trizen-blue hover:bg-trizen-blue/90 transition-all hover:scale-105 text-white button-text uppercase tracking-wide w-full sm:w-auto"
              >
                Know More
              ...
              <TabsList className="mt-8 md:mt-12 flex flex-wrap gap-4 md:gap-8 bg-transparent p-0 overflow-x-auto pb-2 md:pb-0 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide">
                {tabContent.map((tab) => (
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
                    className={`navbar-link font-subheading font-medium whitespace-nowrap ${activeTab === tab.id ? 'text-trizen-purple' : ''}`}
                  >
          ...
          <div className="flex justify-center md:justify-end md:col-span-5 order-1 md:order-2 mb-6 md:mb-0">
